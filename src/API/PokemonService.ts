...
import { InfoProps, Pokemon, PokemonTypeColor, PokemonTypeColors, PokemonTypeProps, StatProps } from "../models/Pokemon";
...
export default class PokemonService {
  static async getAll(paging: Paging) {
    const response = await axios.get(`https://pokeapi.co/api/v2/pokemon?offset=${paging.page*paging.limit}&limit=${paging.limit}`);

    const pokemons: Pokemon[] = []
    await Promise.all(
      response.data.results.map(async (pokemon: any) => {
        const pokemonResponse = await axios.get(pokemon.url)
        const {sprites, id, name, stats, types, base_experience, height, weight} = pokemonResponse.data
